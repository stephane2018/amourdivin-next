...
export function createAudioplayer(
  ...
  function getCurrentTrackMetadata(): TrackMetadata | null {
    if (currentTrackIndex < playlist.playlist.length) {
      return playlist.playlist[currentTrackIndex];
    } else {
      return null;
    }
  }
  ...
  function computeSubsequentTrackIndex(): number {
    return (currentTrackIndex + 1) % playlist.playlist.length;
  }

  function computeRandomTrackIndex(): number {
    if (playlist.playlist.length === 1) return 0;
    const index = Math.floor(Math.random() * (playlist.playlist.length - 1));
